    ...
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-slate-950/70 border-b border-white/10">
        <nav className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm">
            ...
            <a href="#contact" className="px-3 py-1.5 rounded-lg bg-cyan-500 text-slate-950 font-semibold hover:brightness-110">Apply</a>
          </div>
        </nav>
      </header>
      ...
      <section className="relative overflow-hidden wave-top">
        ...
        <div className="mx-auto max-w-6xl px-4 pt-16 pb-20 md:pt-24 md:pb-28 relative">
          <Reveal><div className="mb-4"><Image src="/logo_white_128.png" alt="Coach logo" width={96} height={96} className="drop-shadow-xl" priority /></div></Reveal>
          <Reveal delay={100}><p className="small text-cyan-300/80">603evo • Specialized Training</p></Reveal>
          <Reveal delay={150}><h1 className="h1 mt-3">Elevate Your Game with <span className="text-cyan-400">Elite Off‑Season Coaching</span></h1></Reveal>
          <Reveal delay={200}><p className="mt-5 max-w-2xl text-slate-300">
            Stop wishing. Start working. Transform into a complete lacrosse player with specialized, small‑group
            training led by regionally renowned coach <span className="font-semibold">Gerry Holly</span>.
          </p></Reveal>
          <Reveal delay={250}>
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <a href="#contact" className="btn btn-primary text-center">Apply for December–March Sessions</a>
              <a href="#highlights" className="btn btn-ghost text-center">See Program Highlights</a>
            </div>
          </Reveal>
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          ...
          <div className="card">
            <div className="glass rounded-2xl p-4">
              <p className="text-sm text-slate-400">“Coach Holly’s training gave my son the confidence and speed to make varsity. The attention to detail is next‑level.”</p>
      ...
      <section id="packages" className="mx-auto max-w-6xl px-4 py-16">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            ['Design & Install', '4-week skill block', 'Small group pods', '399'],
            ['Managed Care', 'Monthly sessions', 'Progress reports & video', '499'],
            ...
            <Reveal key={i} delay={i*80}>
              <div className="card p-6">
                ...
                <a href="#contact" className="mt-4 inline-block btn btn-primary">Apply</a>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      {/* COACH BIO */}
      <section id="coach" className="py-16 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-y border-white/10">
        <div className="mx-auto max-w-6xl px-4 grid md:grid-cols-2 gap-10 items-center">
          <Reveal>
            <div className="rounded-3xl overflow-hidden ring-1 ring-white/10">
              <Image src="/logo_white_128.png" alt="Coach icon" width={256} height={256} className="w-full h-auto object-contain bg-slate-950 p-10" />
            </div>
          </Reveal>
          <Reveal delay={120}>
            <div>
              ...
              <p className="mt-4 text-slate-300">
                Regionally renowned lacrosse veteran with a track record of building confident, explosive athletes.
                Sessions emphasize perfect fundamentals, game-ready conditioning, and decision-making IQ.
              </p>
        ...
        <div className="rounded-3xl glass p-6 md:p-10">
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="rounded-2xl bg-slate-900/60 border border-white/10 p-6">
              ...
              <a href="#contact" className="mt-3 inline-block px-5 py-3 rounded-xl bg-cyan-500 text-slate-950 font-semibold hover:brightness-110">Apply for a Spot</a>
              <p className="mt-4 text-xs text-slate-500">Limited capacity. Acceptance prioritized for commitment and fit.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* GALLERY */}
      <section id="gallery" className="py-16 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-y border-white/10">
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4">
            {['/flyer.png','/logo_white_128.png','/logo_white_64.png'].map((src, i) => (
              <div key={i} className="relative overflow-hidden rounded-2xl ring-1 ring-white/10">
                <Image src={src} alt={`gallery ${i}`} width={800} height={600} className="w-full h-full object-cover" />
              </div>
            ))}
          </div>
        </div>
          ...
          <div>
            ...
            <div className="mt-6 space-y-3 text-slate-200">
              <a href="mailto:lax@603evo.com" className="flex items-center gap-3 hover:text-cyan-300">
                <span className="inline-grid h-8 w-8 place-items-center rounded-lg bg-white/10 border border-white/10">@</span>
                lax@603evo.com
              </a>
              <a href="https://instagram.com/lax.603evo" target="_blank" rel="noreferrer" className="flex items-center gap-3 hover:text-cyan-300">
                <span className="inline-grid h-8 w-8 place-items-center rounded-lg bg-white/10 border border-white/10">IG</span>
                @lax.603evo
              </a>
            </div>
            <p className="mt-8 text-xs text-slate-400">Middle School Males • High School Males (Freshman–Senior)</p>
          </div>
          <form className="rounded-2xl glass p-6 grid gap-4" action="https://formspree.io/f/mvoyzqjg" method="POST">
            <div>
              <label className="text-sm text-slate-300">Athlete Name</label>
              <input name="name" required className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-slate-300">Parent Email</label>
                <input type="email" name="email" required className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
              <div>
                <label className="text-sm text-slate-300">Phone</label>
                <input name="phone" className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-slate-300">Age / Grade</label>
                <input name="age" className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
              <div>
                <label className="text-sm text-slate-300">Position</label>
                <input name="position" className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
            </div>
            <div>
              <label className="text-sm text-slate-300">Goals / Notes</label>
              <textarea name="notes" rows={4} className="mt-1 w-full rounded-lg bg-slate-900/60 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
            </div>
            <button className="mt-2 inline-flex justify-center px-5 py-3 rounded-xl bg-cyan-500 text-slate-950 font-semibold hover:brightness-110" type="submit">Submit Application</button>
            <p className="text-xs text-slate-500">Submitting sends details to the coaching staff for review. You’ll get a reply by email.</p>
          </form>
        </div>

        {/* Sticky mobile CTA */}
        <div className="sm:hidden sticky-apply mx-auto max-w-6xl px-4 mt-4">
          <a href="#contact" className="btn btn-primary w-full text-center">Apply Now</a>
        </div>
      </section>
      ...
      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-6xl px-4 py-10 text-sm text-slate-400 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <a href="mailto:lax@603evo.com" className="hover:text-cyan-300">lax@603evo.com</a>
            <span className="opacity-40">•</span>
            <a href="https://instagram.com/lax.603evo" target="_blank" rel="noreferrer" className="hover:text-cyan-300">Instagram</a>
            <span className="opacity-40">•</span>
            <a href="#" className="hover:text-cyan-300">www.603evo.com</a>
          </div>
        </div>
        <div className="mx-auto max-w-6xl px-4 pb-10 text-xs text-cyan-300 text-center">
          Designed by <a href="https://www.NubbleTechnology.com" target="_blank" rel="noreferrer" className="hover:text-cyan-200 underline decoration-cyan-400/40">Nubble Technology</a>
        </div>
      </footer>
    </div>
  );
